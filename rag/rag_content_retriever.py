...
class RAGContentRetriever:
    """
    Handle end-to-end answer retrieval in a Retrieval-Augmented Generation (RAG) system.
    ...
    Integrates three main components:
      ...
      3. Generative model for producing answers conditioned on retrieved context.

    Usage:
        retriever = RAGContentRetriever(cross_encoder, generative_model)
        context, answer = retriever.find_query_response(rag_corpus, query)
    """
    ...
    def __init__(self, cross_encoder, generative_model):
        ...
        Args:
            ...
            generative_model: Generative model (e.g., Gemini) accepting a prompt (str) and returning generated text (str).
        """
    ...
    def _cross_encoder_find_top_similar_items(
        ...
        for index, score in most_related_items_index_score_pairs:
            ...
            heapq.heappush(
                top_chunks_idx_to_metadata[document_id],
                (start_word_index, rag_corpus.chunked_data[index], score),
            )
    ...
    def find_top_similar_items(
        ...
        """
        Retrieve top similar passages to the query (first finding top candidates using biencoder, then rearranging them using cross-encoder).
        ...
        Returns:
            ...
            sorted by descending relevance.
        """
        biencoder_top_passage_chunk_indices = self._biencoder_find_top_similar_items(
            query, biencoder_top_k, rag_corpus
        )
        cross_encoder_top_similar_items = self._cross_encoder_find_top_similar_items(
            query,
            rag_corpus,
            biencoder_top_passage_chunk_indices,
            cross_encoder_top_k,
    ...
    def find_query_response(
        ...
        This method performs the following steps:
          1. Use the bi-encoder FAISS index to find top-k relevant passages.
          2. Re-rank those passages using a cross-encoder for better precision.
          3. Merge overlapping chunks.
          4. Construct a prompt combining the query and top contexts.
          5. Generate a response using the generative model.
        ...
        """
        top_similar_items = self.find_top_similar_items(
            rag_corpus,
            query,
            biencoder_top_k,
            cross_encoder_top_k,
        ...
        prompt = rag_based_instruction + f"\nContext: {context}\nQuestion: {query}"
